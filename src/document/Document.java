...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		boolean lastCharIsE = word.charAt(wordLength-1) == 'e' ||
							  word.charAt(wordLength-1) == 'E';
		if (wordLength > 1) { 
			char secondOfLast = word.charAt(wordLength - 2);
			secondIsVowel = this.isVowel(secondOfLast);
		}
		List<String> letters = this.getTokens("[yaeiouYAEIOU]+", word);
		int lettersLength = letters.size();
		if (lastCharIsE && lettersLength > 1 && !secondIsVowel) {
			lettersLength--;
		}
	...
	public double getFleschScore()
	{
		...
		double numSyllabels = (double)this.getNumSyllables();
		double numWords = (double)this.getNumWords();
		double numSentence = (double)this.getNumSentences();
		
		double wordSentence = 1.015 * (numWords / numSentence);
		double syllableWord = 84.6 * (numSyllabels / numWords);
		
		fleshScore = 206.835 - wordSentence - syllableWord;
