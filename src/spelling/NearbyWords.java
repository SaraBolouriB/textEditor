...
//				sb.setCharAt(index, (char)charCode);
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (!queue.isEmpty() && counter != numSuggestions) {
			String searchWord = queue.get(0);
			if (visited.contains(searchWord)) {
				...
				queue.remove(0);
				List<String> newWords = distanceOne(searchWord, true);
				int numNewWords = newWords.size();
				if ((counter + numNewWords) < numSuggestions) {
					...
					int needWords = numSuggestions - counter;
					counter += needWords;
